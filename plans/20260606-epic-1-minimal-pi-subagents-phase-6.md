...
- Pi users can inspect results, transcripts, and failures without reading raw implementation internals.
...
- Surface structured metadata:
  status, usage, duration, transcript path, recent tool activity, model name, stderr, and exit code.
...
- Keep observability strict:
  every run persists an inspectable transcript or JSON event log.
